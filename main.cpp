...
int main(int argc, char *argv[]) {
  ...
  while (getline(fl, line)) {
    ...
    else if (regex_match(line, arr)) 
    {
      ...
      std::string label = line.substr(6, 10);
      ...
      if (scope.front() == "Global") {
        ...
        else if (sym->getData(var)-> scope == "Global"){
          //this variable was already initialized but in the global scope. append scope name
          Stmt * psh_sl = new Push_scl(var + "_" + scope.front(), sym);
          inst_buff->buff.push_back(psh_sl);
        }
        else {
          cout << "\nERROR: psh_scl scope logic did not work";
        }
      }
    } 
      ...
      if (scope.front() == "Global") {
        ...
        if (sym->getData(var)->scope == scope.front()){
          ...
          cout << "\nERROR: pop_scl scope logic did not work";
        }
      }
    } 
